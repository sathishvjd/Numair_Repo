...
AND DATE(TRANSFER.DATE_CREATED) < (DATEADD('day', -14, CURRENT_DATE())) -- excluding recent tx's as they may not be reconciled
...
TOTAL_TX AS (
    ...
    COUNT(DISTINCT ID) AS TOTAL_TX_COUNT
...
FAKE_TX AS (SELECT COALESCE(DEVICE_FINAL,CLIENT_KEY,PREVIOUS_DEVICE_FINAL,PREVIOUS_CLIENT_KEY) AS DEVICE,
                   ...
                   COUNT(IS_NEW_USER = 1) AS NEW_USER
            FROM PREVIOUS_PROCESSED_DATA
...
AND STATE = 'TRANSFERRED'
AND PREVIOUS_STATE != 'TRANSFERRED' -- all status except transferred (for simplicity)
...
SELECT *,(FAKE_CANCEL_TX/TOTAL_TX_COUNT)*100 FROM FAKE_TX
INNER JOIN TOTAL_TX
ON TOTAL_TX.DEVICE = FAKE_TX.DEVICE;