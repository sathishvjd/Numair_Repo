...
SELECT
UPPER(CUSTOMER_CLASS) AS CUSTOMER_CLASS,
COUNT(DISTINCT USER_PROFILE_ID ) AS COUNT_PROFILES,
ROUND(SUM(COST)) AS TOTAL_VERIFICATION_COST,
TOTAL_VERIFICATION_COST / COUNT_PROFILES AS AVERAGE_VERIFICATION_COST
FROM REPORTS.VERIFICATION_USER_COST VUC
INNER JOIN USERS_REQUEST_VERIFICATION URV
    ON URV.PROFILE_ID = VUC.USER_PROFILE_ID
WHERE TRUE
AND UPPER(COUNTRY_CODE_3_CHAR) = 'IND'
AND USER_PROFILE_CREATED >= '2022-08-01'
AND UPPER(CUSTOMER_CLASS) = 'BUSINESS'
and ((COST !=0) or (TOTAL_COST_WITH_OVERHEAD != 0))
GROUP BY 1;
...
AND VERIFICATION!=0  -- ONLY THOSE USERS WHO HAD VERIFICATION COST (OTHERWISE AVERAGE WILL BE UNDERESTIMATED)
GROUP BY 1;

/*
 EXAMPLE

 OBTAINING VERIFICATION COST FOR CST (33189489) FROM VERIFICATION_USER_COST
 */

SELECT * FROM REPORTS.VERIFICATION_USER_COST VUC
WHERE VUC.USER_PROFILE_ID = 33189489;
