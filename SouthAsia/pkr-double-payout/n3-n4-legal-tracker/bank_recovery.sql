...
/*
 ...
 Description: The following query is used to track recoveries via bank transfers for affected transfers in N3/N4 category to whom legal letters were sent on 1st December 2022
 */
...
SELECT REQUEST_ID,SUM(BTL.LINKED_AMOUNT) as RECOVERED_AMOUNT, LISTAGG(DISTINCT BT.CURRENCY,',') AS RECOVERED_CURRENCY,
       CASE
           WHEN MAX(BTL.DATE) < '2022-12-01' THEN 'PRIOR_LEGAL'
           WHEN MAX(BTL.DATE) >= '2022-12-01' THEN 'POST_LEGAL'
           ELSE 'UNKNOWN'
...
AND BALANCE_RESERVATION = FALSE
AND RECOVERY_TYPE = 'PRIOR_LEGAL'