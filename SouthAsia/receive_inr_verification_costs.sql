...
WHERE TRUE
AND PROFILE_ID IN (SELECT USER_PROFILES.USER_PROFILE_ID FROM USER_PROFILES)
...
WITH USER_PROFILES AS(
SELECT DISTINCT UP.USER_PROFILE_ID FROM
REPORTS.REGIONAL_USER_PROFILE_CHARACTERISTICS AS UP
INNER JOIN DEPOSITACCOUNT.DEPOSIT_ACCOUNT  AS DA
    ON DA.profile_id = UP.USER_PROFILE_ID
INNER JOIN DEPOSITACCOUNT.BANK  AS DAB
...
FROM
REPORTS.CUSTOMER_ECON_DATASET_PUBLIC CE
WHERE TRUE
AND CE.USER_PROFILE_ID IN (SELECT USER_PROFILES.USER_PROFILE_ID FROM USER_PROFILES)
